...
### Methodology
The CipherMQ server (`main.rs`) was run on `127.0.0.1:5672`, initializing `ciphermq_queue` and `ciphermq1_exchange` with routing key `ciphermq_key`. A receiver (`Receiver.py`) subscribed to the queue, decrypting messages and storing them in `received_messages.jsonl`. For 3-to-1 tests, three senders (`Sender.py`) sent pregenerated messages from three JSONL files (100,000 messages each), totaling 300,000 messages. Tests scaled from 100 to 100,000 messages per sender. For details, see the [CipherMQ README](README.md) and [Project Architecture](docs/Project_Architecture.md).
